...
def train(env, sb3_algo):
    ...
    while True:
        iters += 1
        totaltimesteps = TIMESTEPS * iters

        model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False)
        model.save(f"{model_dir}/{sb3_algo}_{totaltimesteps}")
...
if __name__ == '__main__':
    ...
    if args.train:
        gymenv = gym.make("Ant-v4", xml_file = r"C:\Users\ianjl\Downloads\ESP3201 GP\ant.xml", ctrl_cost_weight = 1, 
                        contact_cost_weight = 10, healthy_reward = 0.1, render_mode=None)
        train(gymenv, args.sb3_algo)

    if(args.test):
        if os.path.isfile(args.test):
            gymenv = gym.make("Ant-v4", xml_file = r"C:\Users\ianjl\Downloads\ESP3201 GP\ant.xml", ctrl_cost_weight = 1, 
                            contact_cost_weight = 10, healthy_reward = 0.1, render_mode='human')
