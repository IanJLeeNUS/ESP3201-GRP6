...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    @property
    def DistanceFromSugar(self):
        sugarposx = 6
        sugarposy = 6
        distancefromsugar = np.linalg.norm(self.get_body_com("torso")[:2].copy() - [sugarposx, sugarposy])
        return distancefromsugar
    
    @property
    def AtSugar(self):
        return self.DistanceFromSugar == 0
    
    @property
    def proximity_bonus(self):
        sugarposx = 6
        sugarposy = 6
        current_distance = self.DistanceFromSugar
        radius = np.sqrt(sugarposx ** 2 + sugarposy ** 2)
        proximity_bonus = max(0, (radius - current_distance) * 40)
        return proximity_bonus

    @property
    def check_toppling(self):
        w = self.unwrapped.data.qpos[3]
        x = self.unwrapped.data.qpos[4]
        y = self.unwrapped.data.qpos[5]
        z = self.unwrapped.data.qpos[6]
        vector_magnitude = np.linalg.norm([x, y])
        topple_threshold = 0.5
        if vector_magnitude > topple_threshold:
            return True
        return False

    @property
    def get_target_direction_reward(self):
        sugarposx = 6
        sugarposy = 6
        target_vector = [sugarposx, sugarposy] - self.get_body_com("torso")[:2].copy()
        target_direction = target_vector / (np.linalg.norm(target_vector) + 1e-8)

        heading_velocity = self.data.qvel.flat.copy()[:2]
        heading_direction = heading_velocity / (np.linalg.norm(heading_velocity) + 1e-8)

        cos_similarity = np.dot(target_direction, heading_direction)
        
        direction_reward = max(0, cos_similarity) * 10
        return direction_reward

    @property
    def speed_penalty(self):
        joint_velocities = np.abs(self.data.qvel.flat.copy())
        speed_penalty = 0.1 * np.sum(joint_velocities > 5)
    ...
    def terminated(self):
        ...
        if self.is_healthy:
            if self.AtSugar:
                ...
                terminated = True
        elif self._terminate_when_unhealthy:
            good_termination = False
            terminated = True
        return terminated, good_termination
    
    def step(self, action):
        self.current_step += 1
        xy_position_before = self.get_body_com("torso")[:2].copy()
        sugar_distance_before = self.DistanceFromSugar
        ...
        sugar_distance_after = self.DistanceFromSugar

        distance_change = (sugar_distance_before - sugar_distance_after) * 30

        target_direction_reward = self.get_target_direction_reward
        
        if self.check_toppling:
            toppling_reward = -5
        else:
            toppling_reward = 0.5 + (self.current_step / 1000)
        
        terminated, good_termination = self.terminated
        if good_termination:
            good_termination_reward = 10000
        else:
            good_termination_reward = -20
        ...
        rewards = healthy_reward + proximity_bonus + target_direction_reward

        costs = ctrl_cost + contact_cost + speed_penalty

        reward = rewards - costs + good_termination_reward + distance_change + toppling_reward
